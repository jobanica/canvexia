...
export function SupplierDelete({ supplier }: { supplier: SupplierRow }) {
  ...
  if (!armed) {
    return (
      <>
        <button onClick={() => setArmed(true)} className="text-xs text-slate-500 hover:text-red-300">
          Remove
        </button>
        {state.status === "error" && <p className="mt-1 text-xs text-red-300">{state.message}</p>}
      </>
    );
  }
  ...
  return (
    <form action={action} className="flex items-center gap-2">
      ...
      <button disabled={pending} className="text-xs font-semibold text-red-300 disabled:opacity-40">
        {pending ? "Removing…" : "Confirm"}
      </button>
      <button type="button" onClick={() => setArmed(false)} className="text-xs text-slate-500">
        Cancel
      </button>
    </form>
  );
