          ...
          <ul className="divide-y divide-white/10 overflow-hidden rounded-lg border border-white/10 bg-white/[0.04] backdrop-blur-xl text-sm">
            {sales.map((s) => (
              <li key={s.id}>
                <Link
                  href={`/receipts/${s.id}`}
                  className="flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-3 hover:bg-white/[0.06]"
                >
                  ...
                  <span
                    className={`ml-auto tabular-nums ${s.status === "voided" ? "text-slate-500 line-through" : ""}`}
                  >
