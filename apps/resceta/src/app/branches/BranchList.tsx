    ...
    <form action={action} className="rounded-xl border border-white/10 bg-white/[0.04] backdrop-blur-xl p-5">
      ...
      */}
      <label className="mt-3 flex items-center gap-2 text-sm">
        ...
        />
        Open for business
        {branch?.isMain && <span className="text-xs text-slate-500">(the main branch stays open)</span>}
      </label>
            ...
            {rows.map((b) => (
              <tr key={b.id} className={b.isActive ? undefined : "opacity-50"}>
                <td className="px-4 py-2">
                  ...
                  {b.isMain && (
                    <span className="ml-2 rounded bg-white/10 px-1.5 py-0.5 text-xs">main</span>
                  )}
                ...
                <td className="px-4 py-2 text-slate-300">{b.isActive ? "Open" : "Closed"}</td>
