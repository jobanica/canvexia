...
import { employeeClockSelfie } from "@/server/hr/employee-portal";
import type { MyStatus } from "@/server/hr/employee-portal";
...
export function EmployeeClock({ initial }: { initial: MyStatus }) {
  ...
  async function capture() {
    ...
    setBusy(true);
    setError(null);
    const res = await employeeClockSelfie(dataUrl);
    setBusy(false);
    if (res.ok) {
      stopCamera();
      setStatus({
        name: status.name,
        clockedIn: res.status === "in",
        since: res.status === "in" ? new Date().toISOString() : null,
      });
  ...
  return (
    <div className="rounded-tile border border-plum-ink/10 bg-white p-5 text-center">
      ...
      <div className="my-4 aspect-[3/4] overflow-hidden rounded-tile bg-plum-ink/90">
        ...
        {!camOn && <div className="flex h-full items-center justify-center text-sm text-cream/60">Camera off</div>}
      </div>
      ...
      {!camOn ? (
        ...
        <button onClick={capture} disabled={busy} className="w-full rounded-full py-3 font-semibold btn-brand disabled:opacity-60">
