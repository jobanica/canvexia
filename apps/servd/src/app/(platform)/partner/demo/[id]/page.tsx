...
export default async function PartnerDemoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 px-6 py-8">
      <div>
        <Link href="/partner" className="text-sm text-brand-ink/50">
          ← Partner dashboard
        </Link>
      ...
      <div className="rounded-tile border border-brand-primary/20 bg-brand-primary/5 p-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-wide text-brand-ink/45">Live ordering page</p>
          ...
          <div className="shrink-0 text-center">
            ...
            <img src={qr} alt={`QR code for ${s.name}`} className="h-40 w-40 rounded-lg border border-brand-ink/10 bg-white p-2" />
            <a
              href={qr}
              download={`${s.slug}-qr.png`}
              className="mt-2 inline-block rounded-lg border border-brand-ink/15 bg-white px-3 py-1.5 text-xs font-semibold hover:bg-brand-surface"
            >
              ⬇ Download QR
            </a>
            <p className="mt-1 text-[11px] text-brand-ink/45">Scan to open the menu</p>
          </div>
        </div>
      </div>
      ...
      {/* Convert — the point of the whole preview. Once it has a login it's
          ...
          they log in with. */}
      {login.converted ? (
        <div className="rounded-tile border border-brand-primary/30 bg-brand-primary/10 p-4">
          <p className="font-heading font-bold text-brand-ink">✅ Real account</p>
      ...
      {/* Business details */}
      <form action={updatePartnerDemoDetails} className="grid gap-3 rounded-tile border border-brand-ink/10 bg-white p-4 sm:grid-cols-2">
        <input type="hidden" name="restaurantId" value={s.id} />
        <h2 className="font-heading text-sm font-bold uppercase tracking-wide text-brand-ink/55 sm:col-span-2">
          Business details
        </h2>
        ...
        <div className="sm:col-span-2">
          <label className="mb-1 block text-xs font-semibold text-brand-ink/55">Business logo</label>
          <div className="flex items-center gap-3">
            {s.logoUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={s.logoUrl} alt="Current logo" className="h-14 w-14 shrink-0 rounded-lg border border-brand-ink/10 object-cover" />
            ) : (
              <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-brand-surface text-[10px] text-brand-ink/40">
                no logo
              </span>
            )}
          ...
          <p className="mt-1 text-[11px] text-brand-ink/40">JPEG / PNG / WebP, up to 5 MB. Uploading a file replaces the URL.</p>
        </div>

        <div className="sm:col-span-2">
          <button className="rounded-full border border-brand-ink/15 px-4 py-2 text-sm font-semibold hover:bg-brand-surface">
            Save details
          </button>
        </div>
      </form>
      ...
      <div className="space-y-4">
        ...
        {s.categories.map((cat) => (
          <div key={cat.id} className="rounded-tile border border-brand-ink/10 bg-white p-4">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="font-heading font-bold">{cat.name}</h3>
              <form action={deletePartnerCategory}>
                <input type="hidden" name="restaurantId" value={s.id} />
                <input type="hidden" name="id" value={cat.id} />
                <button className="text-xs text-brand-ink/45 hover:text-guava">delete category</button>
              </form>
            </div>

            {cat.items.length > 0 && (
              <ul className="mb-3 divide-y divide-brand-ink/5">
                {cat.items.map((it) => (
                  <li key={it.id} className="flex items-center justify-between gap-3 py-2 text-sm">
                    <div className="flex min-w-0 items-center gap-2">
                      {it.imageUrl ? (
                        ...
                        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded bg-brand-surface text-[10px] text-brand-ink/40">no img</span>
                      )}
                      <div className="min-w-0">
                        <span className={`font-medium ${it.isAvailable ? "" : "text-brand-ink/40 line-through"}`}>{it.name}</span>
                        {it.description && <span className="block text-xs text-brand-ink/45">{it.description}</span>}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <form action={uploadPartnerItemPhoto} className="flex items-center gap-1">
                        ...
                        <button className="rounded border border-brand-ink/15 px-1.5 py-0.5 text-[11px] hover:bg-brand-surface" title="Upload photo">📷</button>
                      </form>
                      <form action={setPartnerItemPhotoUrl} className="flex items-center gap-1">
                        ...
                        <input
                          ...
                          className="w-32 rounded border border-brand-ink/15 px-1.5 py-0.5 text-[10px]"
                        />
                        <button className="rounded border border-brand-ink/15 px-1.5 py-0.5 text-[11px] hover:bg-brand-surface" title="Use image address (no download)">🔗</button>
                      </form>
                      <form action={togglePartnerItem}>
                        <input type="hidden" name="restaurantId" value={s.id} />
                        <input type="hidden" name="id" value={it.id} />
                        <input type="hidden" name="available" value={(!it.isAvailable).toString()} />
                        <button className="rounded border border-brand-ink/15 px-2 py-0.5 text-xs hover:bg-brand-surface">
                      ...
                      <form action={deletePartnerItem}>
                        <input type="hidden" name="restaurantId" value={s.id} />
                        <input type="hidden" name="id" value={it.id} />
                        <button className="text-xs text-brand-ink/45 hover:text-guava">✕</button>
                      </form>
                    </div>
                  </li>
                ))}
            ...
            <form action={addPartnerItem} className="grid gap-2 sm:grid-cols-[1fr_90px_auto]">
              ...
              <input name="name" required placeholder="Item name" className="rounded-lg border border-brand-ink/15 px-2 py-1.5 text-sm" />
              <input name="price" type="number" step="0.01" min="0" placeholder="₱ price" className="rounded-lg border border-brand-ink/15 px-2 py-1.5 text-sm" />
              <button className="rounded-lg bg-brand-primary px-3 py-1.5 text-sm font-semibold text-white">Add item</button>
              <input name="description" placeholder="Short description (optional)" className="rounded-lg border border-brand-ink/15 px-2 py-1.5 text-sm sm:col-span-3" />
              <input name="imageUrl" type="url" placeholder="Paste an image address / URL (optional — no download needed)" className="rounded-lg border border-brand-ink/15 px-2 py-1.5 text-sm sm:col-span-3" />
              <label className="text-xs text-brand-ink/55 sm:col-span-3">
        ...
        {/* Add category */}
        <form action={addPartnerCategory} className="flex gap-2 rounded-tile border border-dashed border-brand-ink/15 bg-white p-3">
          <input type="hidden" name="restaurantId" value={s.id} />
          <input name="name" required placeholder="New category — e.g. BBQ, Drinks, Sides" className="flex-1 rounded-lg border border-brand-ink/15 px-3 py-2 text-sm" />
      ...
      {!login.converted && (
        <form action={deletePartnerDemo} className="border-t border-brand-ink/10 pt-4">
