> **Archived.** This is the original CANVEXIA platform plan — the trace, the Q1–Q9
> decisions and the phase breakdown that produced the monorepo, the partner
> tenancy, the brand engine and Resceta. Every phase in it shipped; the durable
> record of what was decided and why is `decisions.md` (D1–D36).
>
> It was moved here when `implementation_plan.md` in the root was reused for the
> `apps/www` landing page. Kept rather than deleted: it is the only place the
> *reasoning against the original brief* is written down, and several of its
> findings (the Partner model encoding the opposite business model, the zero-cut
> arrangement existing partners keep) still explain code that is running.
