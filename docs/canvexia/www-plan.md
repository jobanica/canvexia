# ARCHIVED — apps/www (the canvexia.com landing page)

This was `implementation_plan.md` for the landing-page build, which is DONE and
deployed. Moved here so the partner-portal plan could take the filename without
destroying it — the same mistake was made once already in this project and is
what `docs/canvexia/platform-plan.md` exists to record.
